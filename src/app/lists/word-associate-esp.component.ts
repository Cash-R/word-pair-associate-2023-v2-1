...
@Component({
  selector: 'app-word-associate-esp',
  template: `
  <div>
    ...
    <div class="next_btn_wrap_absolute" [class.hidden]='nextClick'>
      ...
      [disabled]="nextClick" 
      [routerLink]="['/task-message-esp']" href="">Presione la barra SPACE (ESPACIADORA) para continuar</button>
    </div>
  </div>
  `
})
export class WordAssociateESPComponent implements OnInit, OnDestroy {
  // @Input() words: AddWord[] = [];
  words: AddWord[] = [];
  nextClick = true;
